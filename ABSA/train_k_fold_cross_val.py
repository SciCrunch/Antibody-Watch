...
from models import AOA_CLS_BERT
...
class Instructor:
        ...
        for child in self.model.children():
            if type(child) != BertModel:  # skip bert params
                ...
                if 'lcf' in self.opt.model_name:
                    self.model.bert_spc.load_state_dict(self.pretrained_bert_state_dict)
                    self.model.bert_local.load_state_dict(self.pretrained_bert_state_dict)
                else:
    ...
    def _evaluate_acc_f1(self, data_loader, save_file=None, fid=0):
        ...
        if save_file:
            ...
            if f1 > self.best_acc:
                self.best_acc = f1
                path = 'state_dict/{0}_{1}_best_val_temp'.format(self.opt.model_name, self.opt.dataset)
                torch.save(self.model.state_dict(), path)
                logger.info('>> saved best model: {}'.format(path))
        f1 = metrics.f1_score(t_targets_all.cpu(), torch.argmax(t_outputs_all, -1).cpu(), labels=[0, 1, 2], average='weighted')
        logger.info(metrics.classification_report(t_targets_all.cpu(), torch.argmax(t_outputs_all, -1).cpu(), labels=[0, 1, 2], target_names=['neg', 'neu', 'pos']))
        return acc, f1

    def _get_label_callback(self, dataset, idx):
        return dataset[idx]['polarity']
    ...
    def run(self):
        ...
        for fid in range(self.opt.cross_val_fold):
            ...
            train_data_loader = DataLoader(dataset=trainset, batch_size=self.opt.batch_size,
                                sampler=ImbalancedDatasetSampler(trainset, callback_get_label=self._get_label_callback)) 
            ...
            test_acc, test_f1 = self._evaluate_acc_f1(val_data_loader, save_file=True, fid=fid)
...
def main():
    ...
    parser.add_argument('--dataset', default='antibody_specificity', type=str, help='try absa-antibody, absa-specific or bert')
    ...
    parser.add_argument('--seed', default=2020, type=int, help='set seed for reproducibility')
    ...
    dataset_files = {
        'antibody_specificity': {
            'train': '../datasets/antibody_specificity/dataset-train_seq.txt',
            'test': '../datasets/antibody_specificity/dataset-test_seq.txt'
        }
    }
    input_colses = {
        'aoa_cls_bert': ['text_shared_bert_indices', 'bert_shared_segments_ids'],
    }
    initializers = {
